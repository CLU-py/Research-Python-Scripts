...
#THIS SCRIPT PLOTS THE COVARIANCE BETWEEN PLASMA VELOCITY AND AURORAL BRIGHTNESS ONTO A SECTOR MAP CENTERED ON A POINT INDICATING SUBSTORM LATITUDE AND LONGITUDE
#TO CALL THIS SCRIPT: fileName substorm_lat substorm_lon starttime endtime (vel_out 73 40 201403210400 201403210600)
...
parser.add_argument('substorm_lon', type = int) #longitude of substorm
...
substorm_lon = args.substorm_lon
sttime = args.sttime
ndtime = args.ndtime
...
#==========================================================================================================================================
#==========================================================================================================================================
#create a text file to write time, velocity, brightness, and covariance values to
with open('data_grid.txt', 'w') as f:
    f.write('grid cell, time, velocity, brightness, covariance\n\n')

#==========================================================================================================================================
#==========================================================================================================================================
#define latitude and longitude range based on specified location
lat_north = substorm_lat + 5
lat_south = substorm_lat - 5

lon_range = 50

if substorm_lon + lon_range > 360:
    lon_west = substorm_lon - lon_range
    lon_east = (substorm_lon + lon_range) - 360

elif substorm_lon - lon_range < 0:
    lon_west = (substorm_lon - lon_range) + 360
    lon_west = substorm_lon + lon_range

else:
    lon_west = substorm_lon - lon_range
    lon_east = substorm_lon + lon_range
...
while 1:
    ...
    #write velocity data into text file
    region_indx = (lat_ll >= lat_south) & (lat_ll <= lat_north) & (lon_ll >= lon_west) & (lon_ll <= lon_east)
    vel_region = vel_grid[region_indx]

    with open('data_grid.txt', 'a') as f:
        for velocity in vel_region:
            f.write(f'[0, 0] {rtime} {velocity}\n')
...
for hr in range(n_hours):
    ...
    for i in range(minute_intervals):
        ...
        #write brightness data into text file
        bright_region = bright_grid[region_indx]
        with open('data_grid.txt', 'r') as f:
            lines = f.readlines()

        for i, brightness in enumerate(bright_region):
            line_index = start_index + i #start on line three

            if line_index < len(lines):
                lines[line_index] = lines[line_index].strip() + f' {brightness}\n'

        with open('data_grid.txt', 'w') as f:
            f.writelines(lines)
            print('Writing brightness to file')

        #step fowards in time
        image_time = image_time + dt.timedelta(minutes = 2)
        intervals += 1

        start_index += len(bright_region) #add to start index as time is progressed fowards
...
for interval in range(intervals):
    ...
    if limit <= intervals:
        ...
        for j in range(grid_points):
            ...
            #print velocity and brightness data into text file
        ...
        curr_cov_max = np.max(covariance_grid) #maximum covariance at the given time
        #print(curr_cov_max)

    if curr_cov_max > cov_max:
        cov_max = curr_cov_max

    #print(cov_max)

    #for j in range(len(curr_vel)):
        #covariance_grid_nd[j] = 1/len(curr_vel_nd) * np.sum( (curr_vel_nd[j] - np.mean(curr_vel_nd)) * (curr_bright[j] - np.mean(curr_bright)) )
    ...
    #======================================================================================================================================
    #create figure
    aspect = (max_lon - min_lon) * np.cos(max_lat * np.pi/180) / (max_lat - min_lat)
    fig = plt.figure(figsize = (10/aspect, 10), dpi = 200)

    #======================================================================================================================================
    #define coordinate reference system
    crs=ccrs.AzimuthalEquidistant(central_longitude = substorm_lon,
                                  central_latitude = substorm_lat,
                                  false_easting = 0.0,
                                  false_northing = 0.0,
                                  globe=None)
    ...
    ax.set_extent((lon_west, lon_east, lat_south, lat_north), ccrs.PlateCarree()) #setting limits of projection to be the sector around the substorm
    ...
    anno_lon = substorm_lon + 0.5 * (lon_east - substorm_lon)

    ax.annotate('60', [0, 60], xycoords = transform, xytext = (anno_lon, 60), c = col, clip_on = True, fontsize = 50)
    ax.annotate('70', [0, 70], xycoords = transform, xytext = (anno_lon, 70), c = col, clip_on = True, fontsize = 50)
    ax.annotate('80', [0, 80], xycoords = transform, xytext = (anno_lon, 80), c = col, clip_on = True, fontsize = 50)
    ...
    name = DMY_str + '_' + HM_str + '_covariance_sector.png'
    plt.savefig(name)
