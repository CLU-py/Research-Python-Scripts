...
hhmm_array = datetime_series.dt.strftime('%H%M').values #return the UT in the form hhmm
...
altitude = geocentric_r - r_e #will be plotted on x-axis
...
ax.tick_params(axis = 'x', direction = 'in', length = 6) #place tick marks on inside of spine and size accordingly
...
ax4.set_xticklabels([f'{m:.1f}' for m in altitude[ax.get_xticks().astype(int)]])
...
ax4.tick_params(axis = 'x', which = 'both', length=  0) #hide tick lines
